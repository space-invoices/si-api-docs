...
_Accepts operators, see the [Where operators](#where-operators) section for details._
...
_Only available on models that implement soft delete._
...
Use with limit to return the n closest points. _`maxDistance` and `unit` can also be passed on same level as `near` to configure the search._
